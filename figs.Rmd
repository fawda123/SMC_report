...
toplo <- evdat[, c('id', evvar)] %>% 
  ...
  rename(
    Phosphorus = Phosphorus_as_P_mgPerL,
    `Total Nitrogen` = Nitrogen_Total_mgPerL,
    `Index Score` = scr,
    `Channel Type` = chcls
  ) %>% 
  dplyr::select(id, ind, `Channel Type`, tax, `Index Score`, thr, Phosphorus, `Total Nitrogen`) %>% 
  filter(!`Channel Type` %in% 'All or partially earthen' & !is.na(`Channel Type`))

ggplot(toplo, aes(x = Phosphorus, y = `Index Score`, colour = `Channel Type`)) +
...
ggplot(toplo, aes(x = `Total Nitrogen`, y = `Index Score`, colour = `Channel Type`)) +
  geom_point(alpha = 0.7, size = 1.5) +
  geom_smooth(method = 'lm') +
  geom_hline(aes(yintercept = thr), linetype = 'dashed') +
  facet_grid(ind ~ tax) +
