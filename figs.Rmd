...
```{r}
evvar <- list(
  varorg = c('Phosphorus_as_P_mgPerL', 'Nitrogen_Total_mgPerL', 'SpecificConductivity_uSPercm'), 
  varnew = c('Phosphorus (mg/L)', 'Total Nitrogen (mg/L)', 'Conductivity (mS/cm)')
)
...
toplo <- indat %>% 
  ...
  mutate(CSCI = cut(CSCI, breaks = c(-Inf, 0.79, Inf), labels = c('Lo', 'Hi'))) %>% 
...
for(i in seq_along(names(toplo))){
  ...
  pcrm <- ggplot(tmp, aes(x = bug_alg, y = CRAM)) + 
    geom_boxplot(fill = 'lightgrey', alpha = 0.8) + 
    theme_bw() + 
    scale_x_discrete('CSCI / ASCI') + 
    ggtitle(cmb)
...
grid.arrange(pcrm1, pnut1, ncol = 2)
```

#### Soft-bodied, O/E
```{r, fig.height = 3, fig.width = 8}
grid.arrange(pcrm2, pnut2, ncol = 2)
```

#### Hybrid, O/E
```{r, fig.height = 3, fig.width = 8}
grid.arrange(pcrm3, pnut3, ncol = 2)
```

#### Diatom, pMMI
```{r, fig.height = 3, fig.width = 8}
grid.arrange(pcrm4, pnut4, ncol = 2)
```

#### Soft-bodied, pMMI
```{r, fig.height = 3, fig.width = 8}
grid.arrange(pcrm5, pnut5, ncol = 2)
```

#### Hybrid, pMMI
```{r, fig.height = 3, fig.width = 8}
grid.arrange(pcrm6, pnut6, ncol = 2)
```