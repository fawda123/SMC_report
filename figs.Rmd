...
ggplot(toplo) + 
  ...
  geom_point(aes(x = lon, y = lat, fill = `Index\nscore`), pch = 21, colour = 'black', alpha = 0.7) +
  facet_wrap(tax ~ ind, ncol = 2) + 
  coord_map() + 
  scale_fill_gradientn(colors = rev(tycol)) + 
  theme_minimal() +
...
evvar <- c('OrthoPhosphate_as_P_mgPerL', 'Ammonia_as_N_mgPerL')
invar <- 'pMMI'
txvar <- 'Soft-bodied'
...
mmplot(stat.data = toplo,
  map.data = polys,
  panel.types = c('labels', 'dot', 'dot', 'dot', 'map'),
  panel.data = list('SMC_Name', 'scr', 'evvar1', 'evvar2', NA), 
  grouping = 3,
  ord.by = 'scr',  
  median.row = F,
  colors = rev(tycol), 
